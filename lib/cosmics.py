...
class cosmicsimage:
    ...
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0, sigclip = 5.0, sigfrac = 0.3, objlim = 5.0, satlevel = 50000.0, verbose=True, BAM=False):
        """
    ...
    def clean(self, mask = None, verbose = False):
        """
    ...
    def findsatstars(self, verbose = False):
        """
    ...
    def run(self, maxiter = 4, verbose = False):
        ...
        if self.satlevel > 0 and self.satstars == None:
            self.findsatstars(verbose=False)

        if verbose is True : print("Starting %i L.A.Cosmic iterations ..." % maxiter)
        for i in range(1, maxiter+1):
            if verbose is True: print("Iteration %i" % i)

            iterres = self.lacosmiciteration(verbose=verbose)
            if verbose is True: print("%i cosmic pixels (%i new)" % (iterres["niter"], iterres["nnew"]))
...
def fromfits(infilename, hdu = 0, verbose = False):
